...
-----------------------
Standalone:   python3 run.py           → http://localhost:8000
From Streamlit: launched automatically on port 8001
"""
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        # Suppress access logs, only show errors
        if args and str(args[1]) not in ('200', '304'):
            print(f"  {args[0]} {args[1]}", file=sys.stderr)

    def do_GET(self):
        # ── Proxy: /api/chart?sym=CL=F ─────────────────────────────────────
